...
export const DockApplication: React.FC<{ application: Application }> = ({ application }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center  h-full relative pb-2" key={application.name}>
      <img
        draggable={false}
      ...
      {runningApps.find((app) => app.id === application.id) && (
        <div className="rounded-full w-1 h-1 bg-[#383838] absolute bottom-0.5 left-1/2 -translate-x-1/2 mix-blend-plus-darker DOT" />
      )}
    </div>
  );
};
